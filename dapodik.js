...
app.post(
  ...
  [
    ...
    body("nisn")
      ...
      .withMessage("NISN harus terdiri dari 10 digit angka")
      .isNumeric()
    ...
    check("nokk", "No.KK tidak valid!")
      .isLength({ min: 16, max: 16 })
      .withMessage("No.KK harus terdiri dari 16 digit angka")
      .isNumeric()
      .withMessage("No.KK tidak valid!"),
  ...
  async (req, res) => {
    ...
    if (!errors.isEmpty()) {
      ...
      const newSiswa = new Siswa({
        ...req.body,
        tgl_masuk: req.body.tgl_masuk.toISOString().split("T")[0], //? Format YYYY-MM-DD
      });
      await newSiswa.save(req.body);
      //? Kirimkan flash message
      req.flash(
        "msg",
        `Data Siswa/i atas nama ${req.body.nama} berhasil ditambahkan`
      );
...
app.delete("/data-siswa/:nisn", isAuthenticated, (req, res) => {
  ...
  Siswa.deleteOne({ nisn: nisn }).then((result) => {
    req.flash("msg", `Data siswa/i atas nama ${req.body.nama} berhasil dihapus`);
    res.status(200).send();
  });
});
  ...
  async (req, res) => {
    ...
    if (!errors.isEmpty()) {
      ...
      await Siswa.updateOne(
        ...
        //! Kirimkan flash message
        req.flash("msg", `Data siswa/i atas nama ${nama} berhasil diupdate`);
        res.redirect("/data-siswa");
      });
    }
  }
